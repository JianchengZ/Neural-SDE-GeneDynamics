...
# global_dict={'16':10,'11':15,'8':20,'6':30,'31':5,'3':75}
...
TRAINPAS = torch.load('results/repeat%d/en_de_temp%d/res%d_%d.pt'%(repeat,batch_hyper, batch_hyper,args.t_size))
TRAIN0 = []
...
encoder = torch.load(r'results/repeat%d/en_de_temp32/big_loss_encoder2000.pth'%repeat , map_location=torch.device('cpu'))
...
for k in tqdm(range(32*(32//batch_hyper))):
    ...
    ys_pred = (torch.sum(ys_pred[:, :batch_hyper, [4, 9]], dim=2)).T
    tmpd = TESTDATA[k]
    truth_data = pd.DataFrame(data=[[float(tmpd[i, j]) for j in range(t_size)] for i in range(batch_hyper)])
    truth_data.to_csv('results/repeat%d/en_de_temp%d/real_truth_traj%d.csv' % (repeat,batch_hyper, k), header=True, index=False)
    predict_data = pd.DataFrame(data=[[float(ys_pred[i, j]) for j in range(t_size)] for i in range(batch_hyper)])
    predict_data.to_csv('results/repeat%d/en_de_temp%d/real_pred_traj%d.csv' % (repeat,batch_hyper, k), header=True, index=False)
