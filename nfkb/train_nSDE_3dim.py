...
for i0 in tqdm(range(epoch), colour='green'):
        ...
        if i0 % 100 == 0:
            torch.save(model.encoder, "three_dim/repeat%d/en_de_temp%d/big_loss_encoder%d.pth" % (repeat,batch_hyper,(i0)))
...
for aA in tqdm(range(121)):
    ...
    A=randomindex[aA]
    IDX.append(A)
    model = mytest(A,L0, L00, model, TEST0[aA], TESTDATA[aA], TESTPAS[aA],
                   'three_dim/repeat%d/en_de_temp%d/ground_truth_verify_traj%d.csv' % (repeat,batch_hyper,aA),
                   'three_dim/repeat%d/en_de_temp%d/predict_verify_traj%d.csv' % (repeat,batch_hyper,aA),
                   'three_dim/repeat%d/en_de_temp%d/ground_truth_verify2_traj%d.csv' % (repeat,batch_hyper,aA), 'three_dim/repeat%d/en_de_temp%d/predict_verify2_traj%d.csv' % (repeat,batch_hyper,aA))
np.save(r'three_dim/repeat%d/en_de_temp%d/noise1%d.npy'%(repeat,batch_hyper,batch_hyper),np.array(noise1))
np.save(r'three_dim/repeat%d/en_de_temp%d/noise2%d.npy'%(repeat,batch_hyper,batch_hyper),np.array(noise2))
np.save(r'three_dim/repeat%d/en_de_temp%d/noise_error%d.npy'%(repeat,batch_hyper,batch_hyper),np.array(L0))
np.save(r'three_dim/repeat%d/en_de_temp%d/w2_error%d.npy'%(repeat,batch_hyper,batch_hyper),np.array(L00))
np.save(r'three_dim/repeat%d/en_de_temp%d/noiseidx%d.npy'%(repeat,batch_hyper,batch_hyper),np.array(noiseidx))
