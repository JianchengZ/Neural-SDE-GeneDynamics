...
class MyLSTM(nn.Module):
    """
    This class creates lstm model that consists of four linear layers with activation functions and two layers of LSTM.
    ...
    def __init__(self):
        ...
        self.lstm = LSTM(input_size=state_size + noise_size, hidden_size=128, batch_first=True, bidirectional=True,
                         num_layers=2, dropout=0.3)
        self.fc1 = nn.Linear(256, 256)
        self.fcfc1 = nn.Linear(256, 256)
    ...
    def forward(self, x):
        ...
        x = torch.tensor(x)
        x = x.to(device)
        out, _ = self.lstm(torch.reshape(x, [batch_size, (state_size + noise_size)]))
        output = out
        out = relu(self.fc1(output))
        out = relu(self.fcfc1(out))
        out = relu(self.fc(out))
    ...
    for k in range(31):
        ...
        for tm in range(361):
            if tm == 0:
                ...
                ys[tm, :, :2] += model(torch.concatenate(
                    (ys_truth[tm - 1].float(), torch.reshape(ys[tm - 1, :, 2].float(), [batch_size, 1])), dim=1))[:, :2]
...
for i0 in tqdm(range(epoch)):
    lstm = train(TRAINDATA, TRAIN0, TRAINPAS, lstm)
    if i0 % 50 == 0:
        torch.save(lstm, r"../results/repeat%d/lstm/lstm%d.pth" % (repeat,i0))
        for A in tqdm(range(31)):
            lstm = _mytest_(lstm, TEST0[A], TESTDATA[A],
                          '../results/repeat%d/lstm/ground_truth_verify_traj%d.csv' % (repeat, A),
                          '../results/repeat%d/lstm/predict_verify_traj%d.csv' % (repeat, A),
                          '../results/repeat%d/lstm/ground_truth_verify2_traj%d.csv' % (repeat, A),
                          '../results/repeat%d/lstm/predict_verify2_traj%d.csv' % (repeat, A))
...
for A in tqdm(range(31)):
    lstm = mytest(lstm, TEST0[A], TESTDATA[A],
                  '../results/repeat%d/lstm/ground_truth_verify_traj%d.csv' % (repeat,A),
                  '../results/repeat%d/lstm/predict_verify_traj%d.csv' % (repeat,A),
                  '../results/repeat%d/lstm/ground_truth_verify2_traj%d.csv' % (repeat,A),
                  '../results/repeat%d/lstm/predict_verify2_traj%d.csv' % (repeat,A))
