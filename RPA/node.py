...
class Drift2(nn.Module):
    """
    This class creates two layer neural network.
    And the input size and hidden size are the same as paper's. 
    """

    def __init__(self):
        super(Drift2, self).__init__()
        self.fc = nn.Linear(state_size + noise_size, 100)
        self.fc_ = nn.Linear(100, 100)
        self.fina = nn.Linear(100, state_size + noise_size)
        self.relu = nn.ReLU(inplace=True)

    def forward(self, t, x):
        newy = x.clone().to(device)
        tmpf = (self.fina(self.relu(self.fc_(self.relu(self.fc(newy))))))  # shape (batch_size, state_size)
...
class NeuralODEFunc(nn.Module):
    """
    This class creates the ode model.
...
def ode_solve(func, t, initial_state):
    """

    :param func: ode model 
    :param t: time
    :param initial_state: trajectories at time_0
    :return: after solving the ode, output the results.
    """
    return odeint(func, initial_state, t, method='euler')
...
optimizer = torch.optim.Adam(neural_ode.parameters(), lr=0.002, betas=(0.9, 0.999), weight_decay=0.005)
epoch = 2001
loss_list = []
ttime = time.time()
g_error_list = []
...
def train(Ys_truth, Y0, Paras, model):
    ...
    for k in range(31):
        ...
        tmpy0[:, state_size:] = paras[3]
        tmpy0[:, :state_size] = y0
        y0 = tmpy0
        ys = ode_solve(neural_ode, ts, y0)
...
def _mytest_(model, y0_test, ys_truth, paras, l1, l2, l3, l4):
    tmpy0 = torch.zeros([y0_test.shape[0], state_size + noise_size]).to(device)
    tmpy0[:, state_size:] = paras[3]
    tmpy0[:, :state_size] = y0_test
    y0_test = tmpy0
    ys = ode_solve(neural_ode, ts, y0_test)
    ys = torch.reshape(ys, (ys.shape[0], ys.shape[1], 3))
    loss = w2_decoupled(ys[:, :, :2], ys_truth[:, :, :2])
    print("test_W2 loss: ", loss)
for i0 in tqdm(range(epoch)):
    neural_ode = train(TRAINDATA, TRAIN0, TRAINPAS, neural_ode)
    if i0 % 50 == 0:
        torch.save(neural_ode, r"../results/repeat%d/ode/node%d.pth" % (repeat, i0))
        for A in tqdm(range(31)):
            neural_ode = _mytest_(neural_ode, TEST0[A], TESTDATA[A], TESTPAS[A],
                                '../results/repeat%d/ode/ground_truth_verify_traj%d.csv' % (repeat, A),
                                '../results/repeat%d/ode/predict_verify_traj%d.csv' % (repeat, A),
                                '../results/repeat%d/ode/ground_truth_verify2_traj%d.csv' % (repeat, A),
                                '../results/repeat%d/ode/predict_verify2_traj%d.csv' % (repeat, A))
...
def mytest(model, y0_test, ys_truth, paras, l1, l2, l3, l4):
    ...
    :param paras: real parameters of testing datasets
...
for A in tqdm(range(31)):
    neural_ode = mytest(neural_ode, TEST0[A], TESTDATA[A], TESTPAS[A],
                        '../results/repeat%d/ode/ground_truth_verify_traj%d.csv' % (repeat, A),
                        '../results/repeat%d/ode/predict_verify_traj%d.csv' % (repeat, A),
                        '../results/repeat%d/ode/ground_truth_verify2_traj%d.csv' % (repeat, A),
                        '../results/repeat%d/ode/predict_verify2_traj%d.csv' % (repeat, A))
